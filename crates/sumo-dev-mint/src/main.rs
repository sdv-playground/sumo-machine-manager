...
//! `factory-reset-issuer` anchor). The key is public *by design* — any dev
...
struct Cli {
    ...
    #[arg(long, default_value = "factory-reset-issuer")]
    issuer: String,
...
mod tests {
    ...
    fn minted_token_satisfies_the_authorizer_contract() {
        ...
        assert_eq!(kid, "factory-reset-issuer", "kid must name the issuer");

        let mut v = Validation::new(Algorithm::ES256);
        v.set_audience(&["rig-1"]);
        v.set_issuer(&["factory-reset-issuer"]);
