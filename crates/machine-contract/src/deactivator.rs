...
//! m7loader).
//!
//! Lives in `machine-contract` so that `component-mgr` (the generic
//! vm-service-stop deactivator) and host machine managers (the RT erase
//! deactivator) can implement the same trait without circular dependencies —
//! the same split as [`crate::bank_activator::BankActivator`].
//!
