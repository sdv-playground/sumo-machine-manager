...
/// Write a complete request to the stream (client side).
pub fn write_request(w: &mut dyn Write, req: &Request) -> io::Result<()> {
    let mut hdr = [0u8; REQUEST_HEADER_SIZE];
    hdr[0..3].copy_from_slice(&VHSM_MAGIC);
    hdr[3] = VHSM_VERSION;
    hdr[4..8].copy_from_slice(&req.op.to_le_bytes());
    hdr[8..12].copy_from_slice(&req.session_id.to_le_bytes());
    hdr[12..16].copy_from_slice(&(req.payload.len() as u32).to_le_bytes());
...
/// Read a complete response from the stream (client side).
pub fn read_response(r: &mut dyn Read) -> io::Result<Response> {
    ...
    let session_id = u32::from_le_bytes([hdr[8], hdr[9], hdr[10], hdr[11]]);
    let payload_len = u32::from_le_bytes([hdr[12], hdr[13], hdr[14], hdr[15]]) as usize;
    let status = u32::from_le_bytes([hdr[16], hdr[17], hdr[18], hdr[19]]);
...
mod tests {
    ...
    // The client-side framing must round-trip against the server-side framing —
    // write_request (client) parses with read_request (server), and vice versa.
    #[test]
    fn client_framing_roundtrips_with_server_framing() {
        let req = Request {
            op: Op::Sign as u32,
            session_id: 9,
            payload: vec![1, 2, 3],
        ...
        let got = read_request(&mut &wire[..]).unwrap();
        assert_eq!(got.op, req.op);
        assert_eq!(got.session_id, req.session_id);
        assert_eq!(got.payload, req.payload);

        let resp = Response::ok(Op::Sign as u32, 9, vec![7, 8]);
        let mut wire = Vec::new();
        write_response(&mut wire, &resp).unwrap();
        let got = read_response(&mut &wire[..]).unwrap();
        assert_eq!(got.op, resp.op);
        assert_eq!(got.session_id, resp.session_id);
        assert_eq!(got.status, resp.status);
        assert_eq!(got.payload, resp.payload);
    }
}
